...
namespace PointsMicroservice.Controllers
{
    ...
    public class PointsController : ControllerBase
    {
        ...
        */
        ...
        public IActionResult GetLikesInTwoDays(int id, DateTime date)
        {
            // List<OfferProvider> newOffers = await GetList();
            var dateMont = date.ToShortDateString();
            var dateMonth = new DateTime(2022, 05, 12).ToShortDateString();
            var d = date.ToShortDateString() == new DateTime(2022, 05, 12).ToShortDateString();
            int count = Likes.Where(c => c.LikeDate.ToShortDateString() == date.ToShortDateString() && c.OfferId == id).Count();
            ...
            return Ok(new {offerId=id, totallikes=totalLikes});
        }  
    }
}